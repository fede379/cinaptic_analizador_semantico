...
from utils import savePickle, getLexicographicTuple, lexicographicSort, loadPickle
...
class GraphGen:
    ...
    def getLexicographicOrderedTuplesList(self, matchingList=[]):
        ...
        try:
            ...
            result = lexicographicSort(mlistIdsOrdered)
        except Exception as e:
            print(e)
            result = []
            pass
    ...
    def isGraphFinished(self, nameGraph = None):
        f = self.loadResults(nameGraph)
        return 'results' in f
    ...
    def deleteGraph(self, nameGraph = None):
        ...
        try:
            p.reset()
            p.MATCH.node('e1', 'Entidad', idGraph=nameGraph).rel('r').node(idGraph=nameGraph)
            p.DELETE(__.r, __.e1)
            self.executeQuery()
        except Exception as e:
            print(e)
